...
input = pd.read_csv('Vancouver_all\\van_listings_detailed.csv', usecols = ['id', 'name', 'description'],encoding='ISO-8859-1')
input.head(10)

input['name'] = input['name'].astype('str')
input['description'] = input['description'].astype('str')

corp = ' '.join(input['name'])
desc_corp = ' '.join(input['description'])
...
input['content'] = input[['name', 'description']].astype(str).apply(lambda x: ' // '.join(x), axis = 1)
input['content'].fillna('Null', inplace = True)
...
tfidf_matrix = tf.fit_transform(input['content'])
...
for idx, row in input.iterrows():
    ...
    similar_items = [(similarity[idx][i], input['id'][i]) for i in similar_indices]
    results[row['id']] = similar_items[1:]

def item(id):
    name   = input.loc[input['id'] == id]['content'].tolist()[0].split(' // ')[0]
    desc   = ' \nDescription: ' + input.loc[input['id'] == id]['content'].tolist()[0].split(' // ')[1][0:165] + '...'
